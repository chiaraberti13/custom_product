	...
	<div id="design_preview" style="min-height:20em;">
		{if isset($customization) AND count($customization) > 0}
            {foreach from=$customization item=design name=design}
			    {include file='./preview_employee.tpl' cpd_group=$design.designs cpd_tags=$design.tags cpd_window=$design.workplace index=$smarty.foreach.design.iteration}
            {/foreach}
		{/if}
	</div>
    <!-- /Design Preview -->
...
<div id="product-part-container" class="">
	<div id="product-part-container">
		<ul id="cpd-parts-bar">
            {if isset($customization) AND count($customization) > 0}
                {foreach from=$customization item=design name=design}
                    {include file='./thumbs_employee.tpl' design=$design.designs index=$smarty.foreach.design.iteration}
                {/foreach}
            {/if}
		</ul>
	</div>
